...
import { Trash2, Download, Folder, X, CheckSquare, Lock } from "lucide-react";
import {
  AlertDialog,
  AlertDialogAction,
  AlertDialogCancel,
...
export default function Gallery() {
  ...
  const moveToVaultMutation = useMutation({
    ...
    onSuccess: () => {
      ...
      toast({
        title: "Success",
        description: `Item(s) moved to vault successfully.`,
      });
    },
    onError: (error) => {
      toast({
        title: "Error",
        description: "Failed to move items to vault.",
        variant: "destructive",
      });
    },
  });
            ...
            <div className="bg-card border border-border rounded-lg p-4 mb-6 flex items-center justify-between animate-in slide-in-from-top-2">
              ...
              <div className="flex items-center gap-2">
                ...
                <Button
                  ...
                  size="sm"
                  onClick={() => {
                    if (window.confirm(`Move ${selectedItems.size} item(s) to encrypted vault?`)) {
                      moveToVaultMutation.mutate(Array.from(selectedItems));
                    }
                  }}
                  data-testid="button-bulk-vault"
                >
