...
export default function DocumentsPage() {
  ...
  const { data: documents = [], isLoading, refetch } = useQuery<Document[]>({
    ...
    queryFn: async () => {
      ...
      const response = await fetch(getApiUrl(`/api/documents?${params}`));
  ...
  const { data: categories = [] } = useQuery<string[]>({
    ...
    queryFn: async () => {
      const response = await fetch(getApiUrl('/api/document-categories'));
  ...
  const uploadMutation = useMutation({
    mutationFn: async (files: FileList) => {
      ...
      formData.append('encryptContent', 'false');
