...
function MediaGridItem({ file, index, isSelected, isFocused, isVaultMode, decryptionKey, onClick, isChecked, onCheckboxClick }: MediaGridItemProps) {
  ...
  // Generate signed URL for encrypted videos (async)
  useEffect(() => {
    if (isVideo && isEncrypted && isVaultMode && decryptionKey) {
      getSignedMediaUrl(file.id, decryptionKey).then(url => {
        setSignedVideoUrl(url);
      });
    }
  }, [isVideo, isEncrypted, isVaultMode, decryptionKey, file.id]);

  // Get video URL for autoplay
  const getVideoUrl = () => {
    if (isVideo && !isEncrypted) {
      return getApiUrl(`/api/media/${file.id}`);
    } else if (isVideo && isEncrypted && isVaultMode && signedVideoUrl) {
  ...
  const getThumbnailUrl = () => {
    if (isEncrypted && isVaultMode && decryptionKey) {
      ...
      return getApiUrl(`/api/media/${file.id}/thumbnail?decrypt=true`);
    } else if (!isEncrypted) {
      return getApiUrl(`/api/media/${file.id}/thumbnail`);
    }
  ...
  return (
    ...
    >
      <div className="relative">
        {/* Use LazyThumbnail for optimized loading */}
        {displayUrl ? (
          <LazyThumbnail
            src={displayUrl}
            ...
            priority={index < 8} // Load first 8 items immediately
            sizes={
              isEncrypted && isVaultMode && decryptionKey
                ? {
                    // For encrypted content, include vault token in size URLs
                    micro: getApiUrl(`/api/media/${file.id}/thumbnail?size=micro&decrypt=true&vt=${encodeURIComponent(decryptionKey)}`),
                    thumb: getApiUrl(`/api/media/${file.id}/thumbnail?size=thumb&decrypt=true&vt=${encodeURIComponent(decryptionKey)}`),
                    small: getApiUrl(`/api/media/${file.id}/thumbnail?size=small&decrypt=true&vt=${encodeURIComponent(decryptionKey)}`),
                    medium: getApiUrl(`/api/media/${file.id}/thumbnail?size=medium&decrypt=true&vt=${encodeURIComponent(decryptionKey)}`)
                  }
                : {
                    // For non-encrypted content, use regular URLs
                    micro: getApiUrl(`/api/media/${file.id}/thumbnail?size=micro`),
                    thumb: getApiUrl(`/api/media/${file.id}/thumbnail?size=thumb`),
                    small: getApiUrl(`/api/media/${file.id}/thumbnail?size=small`),
                    medium: getApiUrl(`/api/media/${file.id}/thumbnail?size=medium`)
                  }
            }
