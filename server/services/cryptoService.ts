...
export class CryptoService {
  ...
  private pbkdf2Iterations = parseInt(process.env.PBKDF2_ITERATIONS || '600000'); // OWASP 2023 recommendation

  /**
   * Derives a key from a passphrase using PBKDF2
   */
  deriveKey(passphrase: string, salt: Buffer): Buffer {
    return crypto.pbkdf2Sync(passphrase, salt, this.pbkdf2Iterations, this.keyLength, 'sha256');
  }
  ...
  hashPassword(password: string): string {
    ...
    const hash = crypto.pbkdf2Sync(password, salt, this.pbkdf2Iterations, 64, 'sha256');
